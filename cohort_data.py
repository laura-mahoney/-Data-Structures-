...
def sort_by_cohort(filename):
    ...
    all_students = [] #all students excluding ghosts
    winter_16 = []
    spring_16 = []
    summer_16 = []
    fall_15 = []
    ...
    for line in cohort_file:
        line = line.rstrip()
        category = line.split("|")
        if category[2]: # if they do specify house then no ghost
            all_students.append(category[0] + " " + category[1])
            all_students = sorted(all_students)
            cohort.add(category[5])

            if  == "Winter 2015"
        else:
            ghosts.append(category[0] + " " + category[1])
    print "students %d ghosts %d" %(len(all_students), len(ghosts))
    cohort_file.close()
